...
if [ ! -f "$PRD_FILE" ]; then
  ...
  echo "Hint: initialize Ralph files first and edit scripts/ralph/prd.json"
  exit 1
fi
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if [[ "$TOOL" == "codex" ]]; then
    # Codex CLI: non-interactive run with workspace sandbox and approval on-request.
    OUTPUT=$(codex exec --full-auto -C "$PROJECT_ROOT" - < "$SCRIPT_DIR/CODEX.md" 2>&1 | tee /dev/stderr) || true
  elif [[ "$TOOL" == "amp" ]]; then
    OUTPUT=$(
      cd "$PROJECT_ROOT" &&
  ...
  # Verify completion from PRD state (source of truth).
  REMAINING=$(jq '[.userStories[] | select(.passes == false)] | length' "$PRD_FILE" 2>/dev/null || echo "999")
  if [[ "$REMAINING" == "0" ]]; then
    echo ""
    echo "Ralph completed all tasks based on PRD state!"
